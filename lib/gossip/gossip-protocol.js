...
class GossipProtocol extends EventEmitter {
  constructor(clawId, gossipBusPath = null) {
    super();
    this.clawId = clawId;
    // Determine gossip bus path:
    // 1. Explicit path passed in (for testing)
    // 2. Repo data/ folder (for MVP test with shared repo)
    // 3. Workspace folder (for installed skill)
    if (gossipBusPath) {
      this.gossipBusPath = gossipBusPath;
    } else {
      const repoPath = path.join(__dirname, '../../data/gossip-bus');
      const workspacePath = path.join(process.env.HOME, '.openclaw/workspace/clawnet/gossip-bus');
      this.gossipBusPath = fs.existsSync(repoPath) ? repoPath : workspacePath;
    }
